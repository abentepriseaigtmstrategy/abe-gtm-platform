...
export function buildReportHTML(strategy) {
  ...
  const wfBar = (label,value,w,cls) => value?`<div class="wb"><div class="wv">${e(safe(value))}</div><div class="wf ${cls}" style="width:${w}"></div><span style="margin-left:2mm;font-size:9px;color:var(--muted)">${label}</span></div>`:'';
  const waterfall = () => {
  ...
  // ── Emails ──
  const emailBlock = (k,i) => { const em=s6[k]; if(!em) return ''; return `<div class="card"><div class="bl">Email ${i+1} — ${e(em.angle||'')}</div><p><strong>Subject:</strong> ${e(em.subject||'—')}</p><p style="white-space:pre-wrap;font-size:10.5px">${e(em.body||'—')}</p><p><span class="tg green" style="margin-top:2mm">CTA: ${e(em.cta||'—')}</span></p></div>`; };
  ...
  <table class="dt">
    <tr><td>Signal Veracity (40%)</td><td class="num">${veracity}/40</td><td>Density and recency of explicit buying signals.</td></tr>
    <tr><td>Market Timing (25%)</td><td class="num">${timing}/25</td><td>Alignment with macro tailwinds and budget cycles.</td></tr>
    <tr><td>ICP Fit (20%)</td><td class="num">${icpFit}/20</td><td>Firmographic and technographic match rate.</td></tr>
    <tr><td>Data Completeness (15%)</td><td class="num">${completeness}/15</td><td>Quality and depth of extraction source.</td></tr>
    <tr style="border-top:1px solid var(--accent)"><td><strong>Overall Fidelity</strong></td><td class="num ha">${confScore}/100</td><td></td></tr>
  </table>
  <div class="confbar"><div class="conffill" style="width:${confScore}%"></div></div>
...
.ww{margin:3mm 0}
.wb{display:flex;align-items:center;margin-bottom:2.5mm}
...
.wf{height:12px;border-radius:6px}
.wfb{background:var(--blue)}
.wfa{background:var(--accent)}
.wfm{background:var(--amber)}
.confbar{background:var(--border);border-radius:4px;height:10px;overflow:hidden;margin:3mm 0}
.conffill{height:100%;border-radius:4px;background:linear-gradient(90deg,var(--accent2),var(--accent))}
