 ...
 * POST /api/login
 *
 * Authenticates user via Supabase, loads profile + org,
 * logs the event, returns full user context + token.
 */
...
export async function onRequestPost(context) {
  ...
  if (!authRes.ok || authData.error) {
    const msg = authData.error_description || authData.error?.message || 'Invalid email or password';
    return errRes(msg, 401, cors);
  }
  ...
  // ── Load user profile (service role — bypasses RLS) ──────────────────────
